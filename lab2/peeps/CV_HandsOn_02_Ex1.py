...
class VCHelper:
    ...
    def compute_proj_view_matrix(self):
        ...
        glUseProgram(self.shader)
        self.projMatrix = glm.ortho(self.min_x, self.max_x, self.
        min_y, self.max_y, -5.0, 15.0)
    ...
    def compute_transf_matrix_triangle(self):
        ...
        self.modelMatrix_triangle = glm.rotate(self.
        modelMatrix_triangle, self.angle, 0, 0, 1.0)
        self.modelMatrix_triangle = glm.translate(self.
        modelMatrix_triangle, self.tx, self.ty, self.tz)
        # code needed here
    ...
    def process_events(self, keys):
        ...
        if keys[pg.K_DOWN]:
            self.ty -= 0.05
            self.angle=180
            if self.ty < self.min_y: # what is this code for?
                self.ty = self.max_y
        if keys[pg.K_RIGHT]:
            self.tx += 0.05
            self.angle=-90
            if self.tx > self.max_x: # what is this code for?
                self.tx = self.min_x
        if keys[pg.K_LEFT]:
            self.tx -= 0.05
            self.angle=90
            if self.tx < self.min_x: # what is this code for?
                self.tx = self.max_x
